...
func (suite *ProcedureSuite) TestFHIRModels(c *C) {
	...
	c.Assert(procedure.Notes, HasLen, 1)
	c.Assert(procedure.Notes[0], Equals, fhir.Annotation{Text: "Procedure, Result: Clinical Staging Procedure"})
	...
	c.Assert(procedure.Notes, HasLen, 1)
	c.Assert(procedure.Notes[0], Equals, fhir.Annotation{Text: "Procedure, Performed: Hospital measures-CABG"})
}
